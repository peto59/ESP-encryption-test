...
void app_main(void) {
	...
	if(prov(&handle, (unsigned char *)buf, PROV_LEN, key, &key_len) != OK){
		printf("prov failed\n");
		return;
	}
	if(key_len != 32){
	...
	len = full_read(STDIN_FILENO, buf, DHKE_LEN + 1);
	buf[DHKE_LEN + 1] = '\0';
	printf("You typed: %s\n", buf);

	if(dhke(&handle, (unsigned char *)buf, DHKE_LEN) != OK){
		printf("dhke failed\n");
		return;
	}
	...
	while(1){
		...
		if (len >= 0) {
			printf("You typed: %s\n", buf);
			if(dec(&handle, (unsigned char *)buf, len, (unsigned char *)decrypted, &dec_len) != OK){
				printf("dec failed\n");
			} else {
				decrypted[dec_len] = '\0';
				printf("dec: %s\n", decrypted);
